import { CustomerCardValidation } from '../../../utlis/validations'
import { serverSupabaseClient } from '#supabase/server'
import { updateCustomerCardDetails } from '~/server/utlis/chargebee'
...
export default defineEventHandler(async (event) => {
  ...
  const cardDetails = await CustomerCardValidation.validateAsync(params)
  if (!cardDetails)
  ...
  const { status, data, error } = await updateCustomerCardDetails(cardDetails, chargebeeUserId)
